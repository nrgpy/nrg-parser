...
from src.parser.mast.utilities.iea_json.iea_models import LoggerMainConfig, IEAMain, MeasurementLocation, \
    MeasurementPoint, LoggerMeasurementConfig, ColumnName, Sensor


def create_iea_json(logger_config_dict_list, sensor_config_dict_list, json_path, logger_oem_id, project_name='Test Project'):
    ...
    for meas_loc in meas_loc_dict_list:
        ...
        print(f'Detected measurement location {ctr} from {meas_loc["date_from"]} to {meas_loc["date_to"]} ..')
        ...
        meas_loc_lmc_df = main_df[
            (main_df['date_from'] >= meas_loc['date_from']) & (main_df['date_to'] <= meas_loc['date_to'])]
        ...
        meas_loc_sensor_df = sensor_df[
            (sensor_df['date_from'] >= meas_loc['date_from']) & (sensor_df['date_to'] <= meas_loc['date_to'])]
...
def get_measurement_locations(df):
    ...
    mdf['loc_change_in_m'] = calculate_distance_m(mdf.latitude_ddeg, mdf.longitude_ddeg, mdf.latitude_ddeg.shift(1),
                                                  mdf.longitude_ddeg.shift(1))

    mdf['moved_beyond_12m'] = np.where(mdf['loc_change_in_m'] > 12, True, False).cumsum()

    # group by location change every change - get date_from and date_to
    ml_grp = mdf.groupby(['logger_name',
                          'measurement_station_type_id',
                          'moved_beyond_12m'], as_index=False, sort=False).agg(
        {"date_from": ["min"], "date_to": ["max"]})
...
def create_logger_main_config(df, logger_oem_id, notes, now):
    if df is None:
        ...
        session = ((df.logger_id != df.logger_id.shift())
        ...
        lmc_grp = df.groupby(['logger_serial_number',
                              'logger_model_name',
                              'logger_id',
                              'logger_name',
                              'offset_from_utc_hrs',
                              'firmware_version',
                              session], as_index=False, sort=False).agg(
            {"date_from": ["min"], "date_to": ["max"]})
        ...
        lmc_list = list(map(lambda x: LoggerMainConfig(logger_serial_number=x['logger_serial_number'],
...
def create_measurement_config(df, notes, now):
    if df is None:
        ...
        for channel_no in channel_list:
            ...
            ch_shift_session = ((channel_df.measurement_unit_id != channel_df.measurement_unit_id.shift())
                                ...
                                | (channel_df.boom_orientation_deg != channel_df.boom_orientation_deg.shift())
                                ).cumsum()

            ch_grp = channel_df.groupby(['connection_channel',
                                         'measurement_unit_id',
                                         'logger_height',
                                         'logger_slope',
                                         'logger_offset',
                                         ...
                                         'column_name_list',
                                         'sensor_type',
                                         ch_shift_session], as_index=False, sort=False).agg(
                {"date_from": ["min"], "date_to": ["max"]})
            ...
            for x in mp_list_dict:
                ...
                meas_point.name = x['measurement_type'] + '_' + str(x['logger_height']) + '_' + str(
                    x['boom_orientation_deg'])
                ...
                measurement_config.logger_stated_boom_orientation_deg = x['boom_orientation_deg']
