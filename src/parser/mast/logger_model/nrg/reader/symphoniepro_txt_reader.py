...
class SymphonieProTxtReader:
    ...
    def __init__(self, txt_filepath):
        """
        Initializes SymphonieProTxt by reading/parsing its file content, both headers and timeseries data.
        Args:
        ...
        try:
            ...
            with open(txt_filepath, 'rt') as file_data:
                for line in file_data:
                    ...
                    for line in file_data:
                        line_ctr += 1
                        str_line = line.strip()
                        if str_line == 'Data':  # means timeseries section, break from section loop
                            break
                        elif str_line == '':  # if empty, it means new section break from section loop
                            break
                        elif str_line.find(':') != -1:  # normal header data
                            split_arr = str_line.split(':', 1)
                            section[split_arr[0].strip()] = util_func.try_string(split_arr[1].strip())
            ...
            try:
                self.timeseries_data = pd.read_csv(txt_filepath, skiprows=self.header_line_ctr,
                                                   sep="\t", encoding="utf-8")
            except:
            ...
            if len(self.timeseries_data) > 0:
                self.timeseries_data['Timestamp'] = pd.to_datetime(self.timeseries_data['Timestamp'], errors='coerce')
    ...
    def get_header(self, header_section_name=None, header=None):
        """
        Helper method that returns symphonie pro txt file in dictionary format if header is not given
    ...
    def get_site_info(self):
        """
        Helper method that returns SymphonieProSiteInfo -symphonie pro basic site information from the headers
        Returns:
        ...
        site_info.site_number = self.get_header(header_section_name=exp_params, header='Site Number')

        # site properties
        site_properties = 'Site Properties'
        site_info.site_description = self.get_header(header_section_name=site_properties, header='Site Description')
        site_info.project = self.get_header(header_section_name=site_properties, header='Project')
        site_info.tower = self.get_header(header_section_name=site_properties, header='Tower')
        site_info.location = self.get_header(header_section_name=site_properties, header='Location')
        site_info.coordinate_system = self.get_header(header_section_name=site_properties, header='Coordinate System')
        ...
        site_info.timezone = self.get_header(header_section_name=site_properties, header='Time Zone')
        site_info.site_units = self.get_header(header_section_name=site_properties, header='Site Units')

        # logger history
        logger_history = 'Logger History'
        site_info.logger_date = self.get_header(header_section_name=logger_history, header='Date')
        site_info.logger_model = self.get_header(header_section_name=logger_history, header='Model')
        site_info.logger_sn = self.get_header(header_section_name=logger_history, header='Serial Number')
        site_info.logger_firmware = self.get_header(header_section_name=logger_history, header='Firmware')

        # iPack History
        ipack_history = 'iPack History'
        site_info.ipack_model = self.get_header(header_section_name=ipack_history, header='Model')
        site_info.ipack_sn = self.get_header(header_section_name=ipack_history, header='Serial Number')
        site_info.ipack_firmware = self.get_header(header_section_name=ipack_history, header='Firmware')
    ...
    def get_iea_logger_main_config(self):
        ...
        logger_main_config_dict['logger_serial_number'] = self.get_header(header_section_name=logger_history,
                                                                          header='Serial Number')
        logger_main_config_dict['logger_model_name'] = self.get_header(header_section_name=logger_history,
                                                                       header='Model')
        logger_main_config_dict['logger_id'] = self.get_header(header_section_name=exp_params, header='Site Number')
        logger_main_config_dict['logger_name'] = self.get_header(header_section_name=site_properties, header='Project')
        ...
        logger_main_config_dict['latitude_ddeg'] = util_func.try_float(
            self.get_header(header_section_name=site_properties,
                            header='Latitude'))
        ...
        logger_main_config_dict['offset_from_utc_hrs'] = util_func.try_float(
            self.get_header(header_section_name=site_properties, header='Time Zone').replace('UTC', '').replace(':',
                                                                                                                '.'))
        logger_main_config_dict['firmware_version'] = self.get_header(header_section_name=logger_history,
                                                                      header='Firmware')
        ...
        for ch_info in self.sensor_history_list:
            ...
            if ch_info['Units'] is not None:
                if ch_info['Units'].lower() in iea_mapping_config.IEA_UNIT_DICT:
                    ...
                    sensor_config_dict['measurement_unit_id'] = iea_mapping_config.IEA_UNIT_DICT[
                        ch_info['Units'].lower()]
                else:
            ...
            sensor_config_dict['logger_slope'] = util_func.try_float(ch_info['Scale Factor'])
            sensor_config_dict['logger_offset'] = util_func.try_float(ch_info['Offset'])
            sensor_config_dict['boom_orientation_deg'] = util_func.try_float(ch_info['Bearing'])
            sensor_config_dict['boom_orientation_deg'] = util_func.try_float(ch_info['Bearing'])
            if ch_info['Type'] == 'Vane':
                sensor_config_dict['vane_dead_band_orientation_deg'] = util_func.try_float(
                    ch_info['Vane Mounting Angle'])
            ...
            match sensor_config_dict['measurement_unit_id']:
                ...
                case 'W/m^2':
                    sensor_config_dict['measurement_type'] = "global_horizontal_irradiance"
            ...
            column_name_list = []
            ch_header = [columns for columns in column_headers if
                         columns.startswith('Ch' + ch_info['Channel'] + '_')]
